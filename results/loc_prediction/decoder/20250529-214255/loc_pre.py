...
def loc_prediction(dataset, pre_model, pre_len, num_epoch, batch_size, device):
    ...
    loss_func = nn.L1Loss()  # Switch to L1 loss for coordinate prediction
    ...
    def pre_func(batch):
        ...
        # Full index sequence still used as model input
        src_seq, _ = create_src_trg(full_seq, pre_len, fill_value=dataset.num_loc)
        src_seq = torch.from_numpy(src_seq).long().to(device)
        imp_seq = torch.full_like(src_seq, fill_value=dataset.num_loc).to(device)
        full_seq = torch.cat([src_seq, imp_seq], dim=-1)

        # Create lat/lng ground truth target
        ...
        out = out.reshape(-1, 2)  # Output should match shape of [B*pre_len, 2]
        return out, label
        ...
        for epoch in range(num_epoch):
            ...
            for i, batch in progress_bar:
                ...
                if (i+1) % test_point == 0:
                    ...
                    progress_bar.set_postfix({'MAE': f"{mae:.6f}"})

            avg_loss = np.mean(epoch_losses)
            avg_mae = np.mean(epoch_maes)
            log.write(f"Epoch {epoch+1} - Loss: {avg_loss:.6f}, MAE: {avg_mae:.6f}\n")
    ...
    print('Best MAE %.6f' % best_mae)
    with open("table.txt", "a") as file:
        file.write(f"{pre_model.pre_model_name}_{num_epoch}|{best_mae:.6f}\n")
...
def fourier_locpred(dataset, pre_model, pre_len, num_epoch, batch_size, device):
    ...
    text = f"{pre_model.pre_model_name}_{num_epoch}|{best_mae:.6f}|{best_mse:.6f}|{best_rmse:.6f}|{best_mape:.6f}"
    with open("table.txt", "a") as file:
